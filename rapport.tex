...
\end{array}
	\]
Or $W$ est engendré par les applications $\sigma_\gamma$, on a donc que $W$ préserve la forme quadratique $B$. Le point 4 suit du point 3, nous l'admettrons dans la suite du document.
\end{proof}
\begin{prop}
La forme quadratique $B$ est définie positive si et seulement si $\Gamma$ est de la forme $A_n,D_n,E_6,E_7$ ou $E_8$.
\end{prop}
\begin{proof}
	Soit $\Gamma$ un graphe. On va d'abord prouver que $\Gamma$ est nécessairement de la forme $A_n,D_n,E_6,E_7$ ou $E_8$. Tout d'abord, remarquons que $\Gamma$ ne peut contenir de sous-graphe d'une des formes suivantes : $\widetilde A_n$ ou $\widetilde D_n$.
