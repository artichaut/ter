...
Rassemblons ici quelques notations que nous utiliserons sans rappel dans la suite du document. Nous noterons $fg$ plutôt que $f\circ g$, la composition entre les fonctions $f$ et $g$, $fg$ ne sera jamais un produit point par point. Nous utiliserons l'abus de notation $x=0$ où $x$ peut représenter des objets de natures diverses, $0$ désignera alors l'élément nulle correspondant à la nature de l'objet $x$. Lorsqu'il n'y a pas d'ambigüité, ou par souci de simplification, nous noterons $V$ plutôt que $(V,f)$ pour une représentation de carquois. Si $V$ et $V'$ sont deux espaces vectoriels, on note $V\oplus V'$ leur somme directe. Si $f$ et $g$ sont deux applications linéaires, on note $f\oplus g$ leur somme directe. Si $A$ et $B$ sont deux objets, on note $A/B$ leur quotient, la définition de quotient varie alors en fonction de la nature de $A$ et $B$. Si $X$ est un ensemble, on note $\Id_X$ l'application identité de $X$ dans lui-même. Si $f$ est une application, on note $\Img f$ son image et $\Ker f$ son noyau. On utilise la flêche $\hookrightarrow$ pour désigner une injection, et la flêche $\twoheadrightarrow$ pour désigner une surjection. On note $[\![1,n]\!]$ l'ensemble des entiers naturels compris entre $1$ et $n$. 
...
\end{defi}
\begin{ex}[Plus petite représentation indécomposable]
\end{ex}
...
% En fait je voulais mettre un puits au début, je me suis planté, mais du coup c'est bien beta si on veut être raccord avec les notations des russes !
	...
	\[
	\begin{array}{ccc}
		 V_{\alpha} & \rightarrow & Z_{\alpha}=\Img \widetilde{h}=\Ker h  \\
