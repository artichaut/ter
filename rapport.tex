...
\begin{enumerate}
	...
	\item Soit $\mathscr M$, (respectivement $\mathscr M'$) l'ensemble des représentations indécomposables (à isomorphisme près) de $\mathscr L(\Gamma,\Lambda)$ (respectivement de $\mathscr L(\Gamma,\Lambda')$). $\widetilde{ \mathscr M} \subset \mathscr M$ l'ensemble des classes des objets $F^{-}_{\alpha_{1}}F^{-}_{\alpha_{2}} \dots F^{-}_{\alpha_{i-1}}(L_{\alpha_{i}})$ pour $i\in [\![1,k]\!]$ et $\widetilde{ \mathscr M'} \subset \mathscr M' $ l'ensemble des classes d'objets $F^{+}_{\alpha_{k}}F^{+_{\alpha_{k-1}}} \dots F^{+}_{\alpha_{i}}(L_{\alpha_{i}})$ pour $i \in [\![1,k]\!]$, alors le foncteur $F^{+}_{\alpha_{k}}F^{+_{\alpha_{k-1}}} \dots F^{+}_{\alpha_{1}}$ est une bijection entre $\mathscr{ M} \setminus \widetilde{\mathscr M}$ et $\mathscr{ M}' \setminus \widetilde{\mathscr{M}'}$.
\end{enumerate}
\end{thm}
